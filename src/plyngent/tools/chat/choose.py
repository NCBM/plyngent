...
def parse_options(options: list[object]) -> list[ChoiceOption]:
    """Normalize a list of strings or dicts into ``ChoiceOption`` list."""
    if not options:
        return []
    out: list[ChoiceOption] = []
    for item_obj in options:
        ...
        if isinstance(item_obj, dict):
            ...
            continue
        msg = "options items must be strings or objects with label/description/value"
        raise TypeError(msg)
    return out
...
async def choose_user(
    ...
    *,
    ...
    ``options`` is a list of strings, or objects with
    ``label`` (required), optional ``description``, optional ``value``.
    ...
    if not parsed and not allow_custom:
        return "error: options must be a non-empty list when allow_custom is false"
    try:
